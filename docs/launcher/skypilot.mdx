...
1. **Create and activate a virtual environment**, then install SkyPilot with the connector for your target infrastructure. Choose one connector install command:

```bash
uv venv
source .venv/bin/activate

uv pip install "skypilot[gcp]==0.8.1"         # Google Cloud
# uv pip install "skypilot[aws]==0.8.1"         # Amazon Web Services
# uv pip install "skypilot[lambda]==0.8.1"      # Lambda Cloud
# uv pip install "skypilot[kubernetes]==0.8.1"  # Any Kubernetes cluster
```
...
Below is an annotated example for fine-tuning Llama-3.2-1B on SQuAD on a GCP spot T4. The checked-in [`examples/llm_finetune/llama3_2/llama3_2_1b_squad_skypilot.yaml`](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/llm_finetune/llama3_2/llama3_2_1b_squad_skypilot.yaml) is a configuration reference, but its commented custom `setup:` example predates the replacement semantics below. If you enable `setup:`, use the complete override shown here.

```yaml
# ── SkyPilot launcher section ─────────────────────────────────────────────
# Removed before the training config reaches the remote VM.
skypilot:
  cloud: gcp                  # aws | gcp | lambda | kubernetes
  ...
  # Optional: replace the default remote setup command.
  # Include the editable install and any extra dependencies in the override:
  # setup: |
...
|---|---|---|
| `cloud` | *(required)* | Cloud provider supported by the pinned install: `aws`, `gcp`, `lambda`, or `kubernetes`. The config accepts `azure`, but its required 0.8.1 connector does not currently resolve. |
...
| `job_name` | Recipe class name | Job and SkyPilot cluster name |
| `setup` | `cd ~/sky_workdir && pip install -e . --quiet` | Remote setup command. A non-empty value replaces this default, so include any required package installation in the override. |
...
| `job_dir` | `./skypilot_jobs` | Local directory for the timestamped training-config snapshot; SkyPilot logs are not written here. |
...
```bash
CLUSTER_NAME="llm_gcp_v100_8gpu"
JOB_ID="1"  # Find job IDs with: sky queue "$CLUSTER_NAME"

sky status                                      # List running clusters and their status
sky logs "$CLUSTER_NAME" "$JOB_ID"              # Stream training logs
sky logs --sync-down "$CLUSTER_NAME" "$JOB_ID"  # Download training logs
ssh "$CLUSTER_NAME"                             # SSH into the VM for debugging
...
1. The `automodel` CLI detects the `skypilot:` key in the YAML and calls `SkyPilotLauncher().launch()`.
...
3. A `sky.Task` is created with:
   ...
   - **setup** — defaults to `cd ~/sky_workdir && pip install -e . --quiet`. A non-empty custom `setup:` value replaces this command rather than being appended, so the override must include any required package installation.
   - **run** — a `torchrun` command pointing at the recipe script and config.
4. `sky.launch()` provisions the VM, runs setup, then executes training. With `detach_run=True`, it returns after job submission without streaming execution logs; use `sky logs` to follow progress.
